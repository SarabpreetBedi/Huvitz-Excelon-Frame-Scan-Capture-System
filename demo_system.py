#!/usr/bin/env python3
"""
Demo System for Huvitz Excelon Frame Scan
...
def create_sample_frame():
    ...
    # Draw a white rectangle (simulating a frame)
    cv2.rectangle(frame, (100, 100), (540, 380), (255, 255, 255), 2)
    
    # Add some text
    cv2.putText(frame, "Huvitz Excelon Frame", (150, 200), 
                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    cv2.putText(frame, "Sample Frame", (200, 250), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
    
    return frame

def demo_frame_processing():
    """Demo frame processing functionality"""
    print("Demo: Frame Processing")
    print("=" * 30)
    ...
    # Process the frame
    scan_data = processor.extract_scan_data(frame)
    
    if scan_data:
        print("✓ Frame processing successful!")
        print(f"  Timestamp: {scan_data['timestamp']}")
        print(f"  Frame shape: {scan_data['frame_shape']}")
        print(f"  Measurements: {scan_data['measurements']}")
        print(f"  Radius points: {len(scan_data['radii'])}")
        
        # Plot the radius data
...
def demo_oma_generation(scan_data):
    """Demo OMa file generation"""
    print("\nDemo: OMa File Generation")
    print("=" * 30)
    ...
    # Generate OMa file
    filename = oma_gen.create_oma_file(scan_data, "demo_scan.oma")
    if filename:
        ...
        if read_data:
            ...
            print(f"  Radius points: {len(read_data['radii'])}")
        else:
            print("✗ Failed to read OMa file")
    else:
        print("✗ Failed to create OMa file")
    
    # Export to JSON
    json_filename = oma_gen.export_to_json(scan_data, "demo_scan.json")
    if json_filename:
...
def demo_visualization():
    ...
    # Plot the frame scan shape
    ax.plot(theta, radii_mm, linewidth=2, color='blue', label='Frame Scan')
    
    # Customize the plot
    ax.set_title('Huvitz Excelon Frame Scan Shape\nSample Data Visualization', 
                 pad=30, fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend()
    
    # Add statistics
    min_radius = np.min(radii_mm)
    max_radius = np.max(radii_mm)
    mean_radius = np.mean(radii_mm)
    
    stats_text = f'Statistics:\nMin: {min_radius:.1f} mm\nMax: {max_radius:.1f} mm\nMean: {mean_radius:.1f} mm'
...
def main():
    """Main demo function"""
    print("Huvitz Excelon Frame Scan System Demo")
    print("=" * 50)
    
    # Demo 1: Frame Processing
    scan_data = demo_frame_processing()
    
    if scan_data:
        # Demo 2: OMa Generation
        demo_oma_generation(scan_data)
        ...
        print("The system is ready for use with your Huvitz Excelon device.")
    else:
        print("\n✗ Demo failed - frame processing error")
